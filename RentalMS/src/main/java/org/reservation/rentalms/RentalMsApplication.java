package org.reservation.rentalms;
...
public class RentalMsApplication {
	...
	private static void rentAndShowStatus(RentalCard card, Item item) {
		card.rentItem(item);
		System.out.println("도서명: ㅅㄷㄴㅅ" + item.getTitle() + " 대여됨");
		showCardStatus(card);
	}
	...
	private static void showCardStatus(RentalCard card) {
		...
		System.out.println("% %" + card.getRentalItemList().stream().map(m
				-> m.getItem().getTitle()).collect(Collectors.toList()));
